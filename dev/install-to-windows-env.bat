...
:: cannot install directly with poetry: some errors on accessing packages, so use pip to preinstall dependencies
:: cannot use "development" install (-e) with pip
pip install .
poetry export --format=requirements.txt --dev --without-hashes >| requirements-dev.txt
pip install --requirement requirements-dev.txt
del requirements-dev.txt


:: make it "development" install
call poetry install
...
echo.
echo SUCCESS: nox test is passed OK
echo.
